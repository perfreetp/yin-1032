...
const operationLogs: LogEntry[] = [
  ...
  { id: 'log-op-006', type: 'operation', timestamp: Date.now() - 60000 * 85, houseId: 'house-villa-001', userId: 'member-owner-001', userName: '张明轩', action: '打开设备', targetType: 'device', targetId: 'dev-light-008', targetName: '儿童房台灯', result: 'success', detail: '亮度85%，色温5000K', ip: randomIP(), userAgent: ua },
  ...
  { id: 'log-op-016', type: 'operation', timestamp: Date.now() - 60000 * 398, houseId: 'house-villa-001', userId: 'member-admin-002', userName: '李雨婷', action: '添加设备', targetType: 'device', targetName: '花园露台灯带', result: 'failed', detail: '设备配对超时，请重试', ip: randomIP(), userAgent: ua },
  ...
  { id: 'log-op-019', type: 'operation', timestamp: Date.now() - 60000 * 520, houseId: 'house-apartment-002', userId: 'member-guest-008', userName: '好友·周子豪', action: '打开设备', targetType: 'device', targetId: 'dev-light-014', targetName: '海景阳台灯', result: 'success', detail: '亮度60%', ip: randomIP(), userAgent: ua },
  { id: 'log-op-020', type: 'operation', timestamp: Date.now() - 60000 * 560, houseId: 'house-apartment-002', userId: 'member-guest-008', userName: '好友·周子豪', action: '调节温度', targetType: 'device', targetId: 'dev-ac-006', targetName: '公寓主卧空调', result: 'success', detail: '制冷模式，25°C', ip: randomIP(), userAgent: ua },
];
...
const deviceLogs: LogEntry[] = [
  ...
  { id: 'log-dev-003', type: 'device', timestamp: Date.now() - 60000 * 52, houseId: 'house-villa-001', action: '设备上线', targetType: 'device', targetId: 'dev-light-009', targetName: '花园露台灯带', result: 'failed', detail: '尝试重连失败，设备持续离线' },
  { id: 'log-dev-004', type: 'device', timestamp: Date.now() - 60000 * 95, houseId: 'house-villa-001', action: '状态变更', targetType: 'device', targetId: 'dev-sensor-007', targetName: '儿童房人体红外', result: 'success', detail: '检测到人体移动' },
  { id: 'log-dev-005', type: 'device', timestamp: Date.now() - 60000 * 132, houseId: 'house-villa-001', action: '电量上报', targetType: 'device', targetId: 'dev-lock-002', targetName: '庭院门锁', result: 'success', detail: '剩余电量68%' },
  ...
  { id: 'log-dev-007', type: 'device', timestamp: Date.now() - 60000 * 218, houseId: 'house-villa-001', action: '故障检测', targetType: 'device', targetId: 'dev-ac-004', targetName: '儿童房空调', result: 'success', detail: '检测到通信模块异常，错误码E-018' },
  ...
  { id: 'log-dev-009', type: 'device', timestamp: Date.now() - 60000 * 302, houseId: 'house-villa-001', action: '状态变更', targetType: 'device', targetId: 'dev-sensor-006', targetName: '花园露台门窗磁', result: 'success', detail: '门已打开' },
  { id: 'log-dev-010', type: 'device', timestamp: Date.now() - 60000 * 345, houseId: 'house-villa-001', action: '录像启动', targetType: 'device', targetId: 'dev-camera-003', targetName: '庭院监控摄像头', result: 'success', detail: '检测到移动，自动录像' },
  { id: 'log-dev-011', type: 'device', timestamp: Date.now() - 60000 * 388, houseId: 'house-villa-001', action: '设备上线', targetType: 'device', targetId: 'dev-light-007', targetName: '主卫镜前灯', result: 'success', detail: '' },
  ...
  { id: 'log-dev-014', type: 'device', timestamp: Date.now() - 60000 * 518, houseId: 'house-villa-001', action: '设备下线', targetType: 'device', targetId: 'dev-light-009', targetName: '花园露台灯带', result: 'success', detail: '网络连接中断' },
  { id: 'log-dev-015', type: 'device', timestamp: Date.now() - 60000 * 560, houseId: 'house-apartment-002', action: '状态变更', targetType: 'device', targetId: 'dev-sensor-009', targetName: '公寓客厅温湿度传感器', result: 'success', detail: '温度26.2°C，湿度65%' },
  { id: 'log-dev-016', type: 'device', timestamp: Date.now() - 60000 * 600, houseId: 'house-apartment-002', action: '状态变更', targetType: 'device', targetId: 'dev-curtain-005', targetName: '海景阳台窗帘', result: 'success', detail: '开合度调整至70%' },
];
...
const alertLogs: LogEntry[] = [
  ...
  { id: 'log-alt-003', type: 'alert', timestamp: Date.now() - 60000 * 200, houseId: 'house-villa-001', userId: 'member-member-003', userName: '张小乐', action: '处理告警', targetType: 'alert', targetId: 'alert-008', targetName: '门窗未关提醒', result: 'success', detail: '已关闭花园露台门' },
  ...
  { id: 'log-alt-008', type: 'alert', timestamp: Date.now() - 60000 * 580, houseId: 'house-villa-001', action: '告警生成', targetType: 'alert', targetId: 'alert-015', targetName: '安防区域异常', result: 'success', detail: '庭院检测到移动，已推送录像至户主' },
  { id: 'log-alt-009', type: 'alert', timestamp: Date.now() - 60000 * 620, houseId: 'house-apartment-002', action: '告警生成', targetType: 'alert', targetId: 'alert-016', targetName: '公寓空调耗电提醒', result: 'success', detail: '公寓主卧空调连续运行8小时' },
];
...
const sceneLogs: LogEntry[] = [
  ...
  { id: 'log-scn-011', type: 'scene', timestamp: Date.now() - 60000 * 740, houseId: 'house-apartment-002', userId: 'member-guest-008', userName: '好友·周子豪', action: '执行场景', targetType: 'scene', targetId: 'scene-apt-sleep', targetName: '度假安睡', result: 'success', detail: '完成3个设备操作' },
];
